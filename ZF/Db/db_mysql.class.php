...
class DB_Sql {
	...
	function connect($Database = "", $Host = "", $User = "", $Password = "") {
		...
		if ($$LinkName) {
			...
			$$LinkName=@mysqli_connect($Host, $User, $Password);
			if ($$LinkName) {
				$this->lctime = time();
				return 1;
			} else {
	...
	function ping(){
		$LinkName=$this->LinkName;
		global $$LinkName;
		$res_ping = @mysqli_db_query($this->Database,"select 'x'",$$LinkName);
		return $res_ping;
	}
	...
	function parse_query_string($Query_String) {
		...
		if (($this->PageSize>0) && ($this->AbsolutePage>0) && eregi("select ",$Query_String)) {
			...
			$strsql.= '/* '.$_SERVER['HTTP_HOST'].':'.$_SERVER['SCRIPT_FILENAME'].' */';

			$rec=@mysqli_db_query($this->Database,$strsql,$$LinkName);
			if (!$rec) {
				$this->Errno = mysqli_errno($$LinkName);
				$this->Error = mysqli_error($$LinkName);
				$this->halt("Invalid SQL(count): ".$Query_String);
				$this->init_pagesize();
				return 0;
			} else {
				$this->RecordCount=mysqli_result($rec,0,"sumrows");
	...
	function query($Query_String) {
		...
		//判断当前连接的时间是否超过25秒，超过了重新new一个
		$ntime = time();
		if ($ntime - $this->lctime >25) {
			$this->close_link();
			$$LinkName = 0;
		}
		...
		//$Query_String.= '/* '.$_SERVER['HTTP_HOST'].':'.$_SERVER['SCRIPT_FILENAME'].' */';
        if (!mysqli_select_db($$LinkName, $this->Database)) {
            echo 'Could not select database';
            exit;
        }
		$this->Query_ID = mysqli_query($$LinkName, $Query_String);
		$this->Row = 0;
		$this->Errno = mysqli_errno($$LinkName);
		$this->Error = mysqli_error($$LinkName);
		if (!$this->Query_ID) {
	...
	function lock($table, $mode="write") {
		...
		$res = @mysqli_db_query($this->Database,$query,$$LinkName);
		if (!$res) {
	...
	function affected_rows() {
		$LinkName=$this->LinkName;
		global $$LinkName;
		return @mysqli_affected_rows($$LinkName);
	}
	...
	function last_insert_id(){
		$LinkName=$this->LinkName;
		global $$LinkName;
		return @mysqli_insert_id($$LinkName);
	}
	...
	function get_row($query , $oMethod = 'A'){
		...
		if($oMethod=='A'){ //返回数组
			if($t = @mysqli_fetch_assoc($this->Query_ID)){
				$Results=$t;
				$this->Row += 1;
			}
		}else if($oMethod=='O'){ //返回对象
			if($t = @mysqli_fetch_object($this->Query_ID)){
				$Results=$t;
				$this->Row += 1;
			}
		}else{
	...
	//例：$user = $DB_Product->get_row("SELECT name,email FROM users"); //MYSQL_BOTH
	function get_results($query , $oMethod = 'A', $fetchType = MYSQLI_ASSOC){
		$this->query($query); //执行SQL
		...
		if($oMethod=='A'){ //返回二维数组
			while($t = @mysqli_fetch_array($this->Query_ID, $fetchType)){
				$Results[]=$t;
				$this->Row += 1;
			}
		}else if($oMethod=='O'){ //对象型数组
			while($t = @mysqli_fetch_object($this->Query_ID)){
				$Results[]=$t;
				$this->Row += 1;
			}
		}else{
    ...
    function get_col($query){
        ...
        while($t = @mysqli_fetch_array($this->Query_ID,MYSQLI_NUM)){
            $Results[]=$t[0];
            $this->Row += 1;
        }
        $this->Errno = mysqli_errno($$LinkName);
	...
	function nextid($seq_name) {
		...
		if ($this->lock($this->Seq_Table)) {
			...
			$this->Seq_Table,
			$seq_name);
			$id = @mysqli_db_query($this->Database,$q, $$LinkName);
			...
			if (!is_array($res)) {
				...
				if ($this->lock("$seq_name")) {
					...
					$rec=@mysqli_db_query($this->Database,$strsql,$$LinkName);
					$currentid=@mysqli_result($rec,0,"maxid");
				} else {
	...
	function get_db_tables(){//得到数据库db_name下的所有表的名字的字符串
		...
		$results=mysqli_list_tables($db_name);
		$table_num=mysqli_affected_rows();
		$table_name="";
		$table_str="";
		for($i=0;$i<$table_num;$i++){
			$table_name=mysqli_tablename($results,$i);
